...
const track = [...socials, ...socials, ...socials, ...socials];
      ...
      <div className="stagger flex flex-col gap-8">
        ...
        <div className="marquee pt-2" style={{ "--motion-marquee": "45s" } as React.CSSProperties}>
          <ul className="marquee__track" aria-label="Redes sociais">
            {track.map((s, i) => (
              <li key={i} className="marquee__item" aria-hidden={i >= socials.length}>
                <a
                  ...
                  aria-label={i < socials.length ? s.label : undefined}
                  className="flex items-center gap-2 font-mono text-sm tracking-mono text-muted-foreground transition-colors hover:text-foreground"
                >
                  {s.icon}
                  {s.label}
                </a>
            ...
            {track.map((s, i) => (
              <li key={`b-${i}`} className="marquee__item" aria-hidden>
                <a
                  href={s.href}
                  target="_blank"
                  rel="noopener noreferrer"
